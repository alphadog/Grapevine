...
public class ReviewsTable implements Table<Review> {
	...
	protected List<Review> findReviews(String query, String[] params) {
		...
		try {
			reviewCursor = tribeDatabase.getReadableDatabase().rawQueryWithFactory(new ReviewCursor.Factory(), query, params, null);
	...
	public Review create(Review newReview) {
		if (newReview != null) {
			...
			try {
				...
				long id = tribeDatabase.getWritableDatabase().insertOrThrow(getTableName(),
		...
		if(reviewList != null && reviewList.size() > 0) {
			...
			try {
				...
				for(Review eachReview : reviewList) {
					...
					tribeDatabase.getWritableDatabase().insertOrThrow(getTableName(), "location_name", dbValues);
				}
