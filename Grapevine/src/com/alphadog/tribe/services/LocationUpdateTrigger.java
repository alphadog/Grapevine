...
public class LocationUpdateTrigger {
    ...
    public LocationUpdateTrigger(Context context, LocationResultExecutor resultExecutor) {
        // By default we'll wait for a minute to get real time updates for
        // location before reverting back to the last known location.
        this(context, 60000, resultExecutor);
    }

    public LocationUpdateTrigger(Context context, long timetoWaitForRealTimeUpdate,
            LocationResultExecutor resultExecutor) {
        this.locationManager = (LocationManager) context.getSystemService(Context.LOCATION_SERVICE);
        this.timetoWaitForRealTimeUpdate = timetoWaitForRealTimeUpdate;
        this.locationResult = resultExecutor;
        Log.d("LocationUpdateTrigger", "We'll wait for " + timetoWaitForRealTimeUpdate
                + "milliseconds before we go with the last known location");
    }
    ...
    private LocationListener gpsLocationListener = new LocationListener() {
        ...
        public void onLocationChanged(Location location) {
            ...
            if (lastKnowLocationTimerTask != null) lastKnowLocationTimerTask.cancel();
    ...
    private LocationListener networkLocationListener = new LocationListener() {
        @Override
        public void onLocationChanged(Location location) {
            Log.d("LocationUpdateTrigger", "We'd like to think we got a fix on the location from Network! : "
                    + location);
            if (lastKnowLocationTimerTask != null) lastKnowLocationTimerTask.cancel();
            locationManager.removeUpdates(this);
    ...
    private TimerTask lastKnowLocationTimerTask = new TimerTask() {
        @Override
        public void run() {
            Log.d("LocationUpdateTrigger",
                    "Seems like neither the GPS not the Network could give a fix on location so we are going with last known.");
            ...
            if (gpsSupported) gpsLocation = locationManager.getLastKnownLocation(LocationManager.GPS_PROVIDER);
            if (networkSupported)
            ...
            if (gpsLocation != null && networkLocation != null) {
                ...
                else locationResult.executeWithUpdatedLocation(networkLocation);
                return;
            }
    ...
    public void fetchLatestLocation() {
        ...
        // It'll wait specified seconds before it'll trigger itself and cancel
        // the real time update listeners.
